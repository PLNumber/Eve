...
이 단어를 활용한 단답형 퀴즈 문제를 **실제 책, 신문, 또는 시험 문제에서 발췌한 것 같은 문장**을 기반으로 작성해주세요.

출력은 반드시 아래 JSON 형식으로 하세요. **추가 설명 없이 JSON만 출력**합니다.

조건:
1. question은 실제 문장 속에서 단어를 빈칸으로 바꾼 문장이어야 합니다. (예: 그는 매우 ___한 성격을 가졌다.)
   - 동사의 경우, 문맥에 맞게 활용된 형태로 문장을 구성하되, 빈칸으로 처리하세요.
2. answer는 반드시 주어진 "{단어}"의 **원형(기본형)**이어야 합니다.
3. distractors는 정답과 형태나 의미가 비슷해 혼동될 수 있는 단어 3개입니다.
4. feedbacks는 distractor 각각에 대해 왜 오답인지 **문맥을 기준으로 설명**합니다.
5. hint는 문맥 또는 의미를 유추할 수 있는 간접적 설명입니다.

```json
{
  "question": "문맥 기반 문장 (빈칸 포함)",
  "answer": "{단어}",  // 동사일 경우 반드시 원형
  "hint": "문맥을 간접적으로 설명",
  "distractors": ["유사어1", "유사어2", "유사어3"],
...
너의 임무는 **오답이 왜 정답이 될 수 없는지**를 설명하는 피드백을 생성하는 거야.

다음 조건을 지켜줘:
1. 피드백은 1문장으로 생성해.
2. 피드백은 오답 단어의 의미/품사/용례를 기준으로 설명해.
3. 정답 단어를 직접 언급하지 마.
4. '문맥상 맞지 않습니다' 같은 뭉뚱그린 표현은 피하고, 구체적으로 써.
5. 피드백은 자연스러운 한국어로 작성해.
